...
ImageAttachment = _ab_mod.ImageAttachment
...
class TestAgentMessage:
    ...
    def test_with_images(self):
        img = ImageAttachment(data="base64data", media_type="image/png")
        msg = AgentMessage(role="user", content="describe this", images=[img])
        assert msg.images is not None
        assert len(msg.images) == 1
        assert msg.images[0].media_type == "image/png"
        assert msg.images[0].data == "base64data"

    def test_image_attachment_optional_filename(self):
        img = ImageAttachment(data="abc", media_type="image/jpeg", filename="photo.jpg")
        assert img.filename == "photo.jpg"
        img2 = ImageAttachment(data="abc", media_type="image/jpeg")
        assert img2.filename is None
...
class TestAgentBackend:
    ...
    def test_supports_vision_enabled(self):
        agent = MockAgentBackend(vision=True)
