...
_TRIGGER_PHRASES: Dict[str, List[str]] = {
    "core": ["comfyui", "node", "connect", "workflow", "basics", "how to", "resolution",
             "sampler", "scheduler", "cfg", "steps", "denoise", "seed", "ksampler",
             "widget", "input", "output", "slot", "connection", "type",
             "api", "format", "json", "class_type", "modify", "change", "settings",
             "blurry", "artifacts", "vram", "tuning", "fix", "improve",
             "add node", "remove node", "connect node", "build", "create"],
    "my_nodes": ["node", "custom node", "install", "what nodes", "available", "find node",
                 "search node", "ipadapter", "controlnet", "detailer", "upscale"],
    "my_models": ["model", "checkpoint", "lora", "vae", "download", "which model",
                  "installed", "pony", "flux", "sdxl", "illustrious", "diffusion model",
                  "unet", "clip", "embedding", "upscale model"],
    "patterns": ["workflow", "template", "pattern", "txt2img", "img2img", "upscale",
                 "create", "make", "build", "pipeline"],
    "prompting": ["prompt", "negative prompt", "trigger", "tags", "score", "quality tags",
                  "how to prompt", "sdxl", "illustrious", "pony", "flux", "sd 1.5", "wan",
                  "noob", "danbooru", "txt2img", "create", "generate", "make", "workflow"],
    "user": [],  # User knowledge — always matched by category filter, not triggers
}
...
class KnowledgeManager:
    ...
    def __init__(self, knowledge_dir: Optional[Path] = None):
        ...
        self.auto_dir = knowledge_dir / "auto"
        self.user_dir = knowledge_dir / "user"
        self.prompting_dir = knowledge_dir / "prompting"
        self._files: List[KnowledgeFile] = []
        self._loaded = False

    def load_all(self) -> None:
        """Scan .md files in knowledge dir, auto dir, and user dir."""
        self._files = []

        # Load from main knowledge dir (quick_reference, workflow_patterns, etc.)
        for md_file in sorted(self.knowledge_dir.glob("*.md")):
            kf = self._parse_file(md_file)
            if kf:
                self._files.append(kf)

        # Load from auto-generated dir (installed nodes, models)
        if self.auto_dir.exists():
            for md_file in sorted(self.auto_dir.glob("*.md")):
                kf = self._parse_file(md_file)
                if kf:
                    self._files.append(kf)

        # Load from prompting guides dir
        if self.prompting_dir.exists():
            for md_file in sorted(self.prompting_dir.glob("*.md")):
                kf = self._parse_file(md_file)
                if kf:
                    self._files.append(kf)

        # Load from user dir (custom user knowledge)
    ...
    def select_relevant(
        ...
        for kf in self._files:
            ...
            # Score by keyword matches (word-boundary to avoid false positives)
            score = 0
